...
ChessGame::doMove(Move& move)
{
    ...
    bool move_has_side_effects = false;

    if (isWhite(move.piece)) {
        player = Player::WHITE;
    } else {
        player = Player::BLACK;
        // A full move is complete after both white and black have moved
        this->num_moves++;
    }

    // Fill in the captured pience, if any
    move.captured_piece = getPieceAtDestinationSquare(move);

    // FIXME: Handle castling

    // Make the move on the board
    board[move.destination_file][move.destination_rank].piece = getPieceAtSourceSquare(move);
    board[move.source_file][move.source_rank].piece = Piece::EMPTY;

    // If there was no capture, and a pawn has not moved, increase the halfmove clock
    if (move.captured_piece == Piece::EMPTY && !isPawn(move.piece)) {
        halfmove_clock++;
    }

    // Check for side effects, and if present store them in the move
    if (move.piece == Piece::WHITE_KING) {
        side_effects.white_king_moved = true;
        side_effects.white_kingside_castle_changed = true;
        side_effects.white_queenside_castle_changed = true;
        this->white_kingside_castle_allowed = false;
        this->white_queenside_castle_allowed = false;
        move_has_side_effects = true;
    } else if (move.piece == Piece::BLACK_KING) {
        side_effects.black_king_moved = true;
        side_effects.black_kingside_castle_changed = true;
        side_effects.black_queenside_castle_changed = true;
        this->black_kingside_castle_allowed = false;
        this->black_queenside_castle_allowed = false;
        move_has_side_effects = true;
    } else if (move.piece == Piece::WHITE_ROOK) {
        if (move.source_file == 1) {
            side_effects.white_queenside_rook_moved = true;
            this->white_queenside_castle_allowed = false;
            move_has_side_effects = true;
        } else if (move.source_file == 8) {
            side_effects.white_kingside_rook_moved = true;
            this->white_kingside_castle_allowed = false;
            move_has_side_effects = true;
        }
    } else if (move.piece == Piece::BLACK_ROOK) {
        if (move.source_file == 1) {
            side_effects.black_queenside_rook_moved = true;
            this->black_queenside_castle_allowed = false;
            move_has_side_effects = true;
        } else if (move.source_file == 8) {
            side_effects.black_kingside_rook_moved = true;
            this->black_kingside_castle_allowed = false;
            move_has_side_effects = true;
        }
        ...
        if (move.source_rank == 2 && move.destination_rank == 4) {
            side_effects.pawn_moved_two_squares = true;
            this->two_square_pawn_push_just_occured = true;
            this->en_passant_target_square_rank = 3;
            this->en_passant_target_square_file = move.destination_file;
            move_has_side_effects = true;
        }
        ...
        if (move.source_rank == 7 && move.destination_rank == 5) {
            side_effects.pawn_moved_two_squares = true;
            this->two_square_pawn_push_just_occured = true;
            this->en_passant_target_square_rank = 6;
            this->en_passant_target_square_file = move.destination_file;
            move_has_side_effects = true;
        }
    }

    // Check if it's a pawn that moved two squares forward
    if (move_has_side_effects) {
        move.side_effects = std::make_unique<SideEffects>(side_effects);
    }
    */
}
...
#if 0
void
ChessGame::doKingsideCastle(Player player)
{
}

void
ChessGame::doQueensideCastle(Player player)
{
}
...
ChessGame::isLegalMove(const Move& move)
{
    ...
    // Figure out which player is making the move
    if (move.piece >= Piece::WHITE_PAWN && move.piece <= Piece::WHITE_KING) {
        player = Player::WHITE;
    } else {
        player = Player::BLACK;
    }

    // Check if that player is now in check after the move
    // FIXME: Same logic as checking if valid king move, need to move, check, then unmove
 ...
 */
bool
ChessGame::isValidPawnMove(const Move& move)
{
...
bool
ChessGame::isValidBishopMove(const Move& move)
{
...
bool
ChessGame::isValidKnightMove(const Move& move)
{
...
bool
ChessGame::isValidRookMove(const Move& move)
{
...
bool 
ChessGame::isValidQueenMove(const Move& move)
{
...
bool
ChessGame::isSquareOccupied(int8_t rank, int8_t file)
{
    return board[rank][file].piece != Piece::EMPTY;
}

bool
ChessGame::isWhite(Piece piece)
{
    return (piece >= Piece::WHITE_PAWN && piece <= Piece::WHITE_KING);
}

bool
ChessGame::isPawn(Piece piece)
{
